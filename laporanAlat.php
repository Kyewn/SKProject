...
<head>    
  <meta charset="UTF-8">
    ...
    <link rel="stylesheet" type="text/css" href="laporanAlat.css">
...
<body>
    ...
    <div class="content">
        ...
        <button id="cetak" class="cetak" type="button" onclick="window.print();">Cetak</button>
        <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>">
            <select id="choice" name="choice">
                <option value="null"><?php echo isset($_POST['choice'])? 'Dipilih: '.$_POST['choice'] : "Pilih Alat: ";?></option>
                <?php
                ...
                while($row = mysqli_fetch_assoc($res)){
                    echo "<option value='".strtolower($row['NAMAALAT'])."'>".$row['NAMAALAT']."</option>";
                }
                ?>
            </select>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $query = "SELECT * FROM peralatan WHERE NAMAALAT = '$alat'";
            //$query1 = "SELECT * FROM kerosakan WHERE NAMA_ALAT = '$alat'";
            $res = mysqli_query($connection, $query);
            //$res1 = mysqli_query($connection, $query1);
            //$bilRosak = mysqli_num_rows($res1);
            if ($row = mysqli_fetch_assoc($res)){       
                echo "<div class='first'>
                <span>Kod Alat:</span><p>P".$row['KODALAT']."</p><br/><br/>
                <span>Nama Alat:</span><p>".$row['NAMAALAT']."</p><br/><br/>
                <span>Bilangan Alat:<p>".$row['BILANGANALAT']."</p></span><br/><br/>
                <span>Jenis Alat:<p>".$row['JENISALAT']."</p></span><br/><br/>
                <span>Didaftarkan Oleh:<p>".$row['PENDAFTAR']."</p></span>
                </div>
                <div class='sec'>
                </div>"; //-$bilRosak
                }
